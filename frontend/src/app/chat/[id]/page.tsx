...
export default function ChatPage(): ReactElement {
  ...
  return (
    <main className="flex min-h-[calc(100vh-57px)] flex-col bg-slate-50">
      <div className="mx-auto flex w-full max-w-3xl flex-1 flex-col px-4 py-6">
        <h1 className="mb-4 text-xl font-semibold">Chat con documento</h1>

        {/* Message list */}
        <div className="flex flex-1 flex-col gap-3 overflow-y-auto rounded-xl border bg-white p-4 shadow-sm">
          {messages.length === 0 && (
          ...
          {isPending && (
            <div className="max-w-[85%] rounded-2xl bg-slate-100 px-4 py-2 text-sm text-slate-400">
              Pensando...
            </div>
          )}
          <div ref={bottomRef} />
        ...
        <form onSubmit={handleSubmit} className="mt-3 flex gap-2">
          <input
            className="flex-1 rounded-lg border bg-white px-4 py-2 text-sm shadow-sm outline-none focus:ring-2 focus:ring-blue-500"
